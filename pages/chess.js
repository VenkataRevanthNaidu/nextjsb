...
function makeGame(div) {
  ...
  const onClick = event => {
   ...
   const [movFrom, movTo] = Object.entries(game.aiMove())[0];

   gameState= game.exportJson();
   // clearing
   document.getElementById(movFrom).innerText = "";
   // adding the new sqaure
   document.getElementById(movTo).innerText = GLYPHS[gameState.pieces[movTo]];
 } else if (selected) {
